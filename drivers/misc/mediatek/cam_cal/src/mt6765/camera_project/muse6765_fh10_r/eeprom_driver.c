	...
	if (pcmdInf != NULL) {
		if (pcmdInf->readCMDFunc != NULL){
			if (pData->dataBuffer == NULL){
				...
				memset(pData->dataBuffer, 0x0, pData->dataLength);//bug577281 zhoumin.wt, add, 2020/9/10, rear camera otp porting
				if (pData->dataBuffer == NULL) {
	...
	case CAM_CALIOC_G_READ:
...
#endif
		//+bug577281 chenbocheng.wt, add, 2020/9/9, camera otp porting
		LOG_INF("%d, SensorID=%x DeviceID=%x\n", __LINE__, ptempbuf->sensorID, ptempbuf->deviceID);//bug577281 zhoumin.wt, modify, 2020/9/10, rear camera otp porting
		...
		if((ptempbuf->deviceID == 0x01)&&(g_eepromMainData != NULL)){
			...
			if((g_eepromMainData->dataBuffer)&&(totalLength <= g_eepromMainData->dataLength)){
				//+bug577281 zhoumin.wt, modify, 2020/11/23, add new lens_id for micro and front camera
				#if 0
