...
//char g_lge_physical_camera[2] = "";
//EXPORT_SYMBOL(g_lge_physical_camera);

static int g_lge_physical_camera = -1;

static int __init lge_physical_camera(char *lge_physical_camera)
{
    int ret = 0;

    ret = kstrtoint(lge_physical_camera, 0, &g_lge_physical_camera);

    if(!ret) {
        printk("lge_physical_camera %d\n", g_lge_physical_camera);
	} else {
        printk("lge_physical_camera no value %d\n", ret);
    }

	return 0;
}
early_param("androidboot.product.lge.physical.camera", lge_physical_camera);
...
{
    if(g_lge_physical_camera == 5) {
        pr_err("show_LGCameraSensorName: rear_camera_name [%s] , front_camera_name [%s] , mono_sensor_name [%s] , macro_sensor_name [%s], depth_sensor_name[%s] \n", rear_sensor_name, front_sensor_name, mono_sensor_name, macro_sensor_name, depth_sensor_name);
        return sprintf(buf, "FCam:%s^^RCam:%s^^RCam1:%s^^RCam2:%s^^RCam3:%s\n", front_sensor_name, rear_sensor_name, mono_sensor_name, macro_sensor_name, depth_sensor_name);
    }
    pr_err("show_LGCameraSensorName: rear_camera_name [%s] , front_camera_name [%s] , macro_sensor_name [%s], depth_sensor_name[%s] \n", rear_sensor_name, front_sensor_name, macro_sensor_name, depth_sensor_name);
    return sprintf(buf, "FCam:%s^^RCam:%s^^RCam1:%s^^RCam2:%s\n", front_sensor_name, rear_sensor_name, macro_sensor_name, depth_sensor_name);
}
