 ...
 * Filename:
 * ---------
 *     fh10_gc02m1b_mono_lhyxmipi_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "fh10_gc02m1b_mono_lhyxmipi_Sensor.h"
...
extern char mono_sensor_name[20];/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = FH10_GC02M1B_MONO_LHYX_SENSOR_ID,
		...
		do {
			*sensor_id = return_sensor_id() + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(mono_sensor_name, "gc02m1bmono_unimage");/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/
...
UINT32 FH10_GC02M1B_MONO_LHYX_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	printk("FH10_GC02M1B_MONO_LHYX_MIPI_RAW_SensorInit.\n");
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}
