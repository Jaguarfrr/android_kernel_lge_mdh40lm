...
extern char rear_sensor_name[20];/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/
...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x1e03810f,       //0x1e03810f // Auto Test Mode
	.pre = {
	...
	.hs_video = {
		...
		.framelength = 4597, 			//record different mode's framelength
		.startx = 0,				    //record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
    ...
    .custom1 = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 230,
		.mipi_pixel_rate = 456960000//1142.4*4/10
	},
	.margin = 4,
	.min_shutter = 4,
...
{
	{ 4672, 3520,	0,	 4, 4672, 3512,  2336, 1756,  4,  2, 2328, 1752, 0, 0, 2328, 1752}, 	// preview (2328 x 1752)//DIFF
	...
	{ 4672, 3520,   0, 676, 4672, 2168,  2336, 1084, 208, 2, 1920, 1080, 0, 0, 1920, 1080}, 	// hs video (1920 x 1080)//DIFF
	{ 4672, 3520,   8, 674, 4656, 2172,  1552, 724,  136, 2, 1280,  720, 0, 0, 1280,  720}, 	// slim video (1280 x 720)
	{ 4672, 3520,	0,	 6, 4672, 3508,  4672, 3508,  8,  2, 4656, 3504, 0, 0, 4656, 3504}, 	// custom1 (4656 x 3504)
};
...
#if HI1634_EEPROM_ENABLE
static kal_uint16 read_sensor_eeprom(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, HI1634_TS_EEPROM);
...
kal_uint16 addr_data_pair_preview_hi1634[] = {
	...
	//Sensor				: Hi-1634B
	//Date					: 2020-07-30
	//Customer		  : Huaqin EH20
	//Image size		: 2328x1752
	//MCLK						: 24MHz
	//MIPI speed(Mbps): 848Mbps x 4Lane
	//Max Fps			: 30.00fps
...
kal_uint16 addr_data_pair_capture_30fps_hi1634[] = {
	...
	//MCLK						: 24MHz
	//MIPI speed(Mbps): 1696Mbps x 4Lane
	//Max Fps			: 30.00fps
	//Pixel order	  : Green 1st(=GR)
	//PDAF type 			: 2 VC / PD-DPC on, Dyn-DPC on
	////////////////////////////////////////////////
	0x0204, 0x0000, 
	...
	0x0F04, 0x0008, 
	0x0B04, 0x00DC, 
	0x0B12, 0x1230, //x_output_size
	0x0B14, 0x0DB0, //y_output_size
	0x0B20, 0x0100, 
	0x1100, 0x1100, 
	0x1108, 0x0202, 
	0x1116, 0x0000, 
...
kal_uint16 addr_data_pair_slim_video_hi1634[] = {
	...
	//Image size		: 1280x720
	//MCLK						: 24MHz
	//MIPI speed(Mbps): 565.33Mbps x 4Lane
	//Max Fps			: 120.03fps
	//Pixel order	  : Green 1st(=GR)
	//PDAF type 			: disable / PD-DPC on, Dyn-DPC on
	////////////////////////////////////////////////
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_custom1_hi1634[] = {
	//Sensor Information////////////////////////////
	//Sensor				: Hi-1634B
	//Date					: 2020-08-20
	//Customer		  : Huaqin EH20
	//Image size		: 4656x3504
	//MCLK						: 24MHz
	//MIPI speed(Mbps): 1142.4Mbps x 4Lane
	//Max Fps			: 23.06fps
	//Pixel order	  : Green 1st(=GR)
	//PDAF type 			: Disable / PD-DPC on, Dyn-DPC on
	////////////////////////////////////////////////
...
{
LOG_INF("hi1634 custom1_video E");
#if MULTI_WRITE
	hi1634_table_write_cmos_sensor(
		addr_data_pair_custom1_hi1634,
		sizeof(addr_data_pair_custom1_hi1634) /
		sizeof(kal_uint16));
#else
...
static int read_eeprom_barcode(void)
{
    ...
    kal_uint8 i;
    kal_uint16 checkSum_LSB = 0;
    kal_uint16 checkSum_MSB = 0;
    if(barcodeFlag != 0x01)
    {
        printk("hi1634 barcodeFlag invalid !\n");
        return -1;
    }
    printk("hi1634  barcodeFlag valid !barcodeFlag =0x%x!\n",barcodeFlag);
    ...
    for (i=0;i < QRCODE_ADDR_LENS; i++)
    {
        ...
        printk("[hi1634 sn_code]%s :[3][0x%x]=[0x%x]",__func__, i,sn_code[i]);
        checkSum += sn_code[i];
    }
    printk("hi1634 sn_code:%s",sn_code);

    checkSum_LSB = (checkSum%65535)& 0xFF;
    checkSum_MSB = (checkSum%65535)>>8;
    if( (checkSum_LSB == read_sensor_eeprom(QRCODE_CHECKSUM)) && 
        (checkSum_MSB == read_sensor_eeprom(QRCODE_CHECKSUM+1)))
    {
        get_hardware_info_data(HWID_MAIN_CAM_SN, sn_code);
        return 0;
    }
    printk("hi1634  barcode checksum error\n");
    return -1;
}
...
static int  get_eeprom_data(EEPROM_DATA *data)
{
	...
	unsigned int checkSum_LSB = 0;
    unsigned int checkSum_MSB = 0;
	for (i =0x00;i <= 0x0E;i++,otp_data++) {
		*otp_data = read_sensor_eeprom(i);
		if(i != 0x00){
			checksum += *otp_data;
			}
		printk("[hi1634 otp]%s :[1][0x%x]=[0x%x]",__func__, i,*otp_data);
	}
	
    checkSum_LSB = (checksum%65535)&0xFF;
    checkSum_MSB = (checksum%65535)>>8;
    
	printk("[hi1634 otp]%s :[2]checksum =[0x%x] LSB=[0x%x] MSB=[0x%x]",__func__, checksum,checkSum_LSB,checkSum_MSB);
	read_eeprom_barcode();
    if( (checkSum_LSB == read_sensor_eeprom(0x0015)) && 
        (checkSum_MSB == read_sensor_eeprom(0x0016)))
    {
		return 0;
	}
	return  -1;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(rear_sensor_name, "hi634rear_txd");/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/
				printk("hi1634 i2c write id : 0x%x, sensor id: 0x%x\n",
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("hi1634 Read id fail,sensor id: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	LOG_INF("hi1634 [open]: PLATFORM:MT6765,MIPI 4LANE\n");
	LOG_INF("hi1634 preview 2328x1752@30fps,848Mbps/lane;"
		"capture 4656x3504@30fps,1696Mbps/lane\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
	...
	sensor_init();
	LOG_INF("SK hynix Hi1634 Sensor Init\n");
	spin_lock(&imgsensor_drv_lock);
...
#if ENABLE_PDAF
	//sensor_info->PDAF_Support = PDAF_SUPPORT_CAMSV; // 2 type2 VC
	sensor_info->PDAF_Support = 2;  // type3 RAW_PDO
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_STREAMING_RESUME:
		...
		case SENSOR_FEATURE_GET_VC_INFO:
				printk("hi1634 SENSOR_FEATURE_GET_VC_INFO %d\n", (UINT16)*feature_data);
            #if 1
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		LOG_INF("hi1634 pdaf SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY scenarioId:%lld\n", *feature_data);
