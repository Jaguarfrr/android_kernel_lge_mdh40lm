...
{
    pr_err("show_LGCameraSensorName: rear_camera_name [%s] , front_camera_name [%s] , wide_sensor_name [%s] , macro_sensor_name [%s], depth_sensor_name[%s] \n", rear_sensor_name, front_sensor_name, wide_sensor_name, macro_sensor_name, depth_sensor_name);
    return sprintf(buf, "FCam:%s^^RCam:%s^^RCam1:%s^^RCam2:%s^^RCam3:%s\n", front_sensor_name, rear_sensor_name, wide_sensor_name, macro_sensor_name, depth_sensor_name);
}
...
MODULE_DEVICE_TABLE(of,gimgsensor_of_device_id);
...
#if 0
static struct platform_device gimgsensor_platform_device = {
    .name   = "image_sensor",
    .id	    = -1,
};
#endif
...
{
	int retn;
	pr_err("[camerahw_probe] start - rev check\n");
#if 0	
	if (platform_device_register(&gimgsensor_platform_device)) {
                PK_DBG("failed to register platform device\n");
                return -ENODEV;
        }

#endif	
	retn = platform_driver_register(&gimgsensor_platform_driver);
	printk(KERN_EMERG " %s platform_driver_register returned = %d\n",__func__,retn);
...
#endif
	pr_err("[camerahw_probe] start exit\n");
