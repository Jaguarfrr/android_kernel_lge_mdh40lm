...
{
	printk(KERN_EMERG"[LGSI Probe success for SLAVE ID 0x%x\n", client->addr);
...
enum IMGSENSOR_RETURN imgsensor_i2c_create(void)
{
	int i;
	int ret;
	printk(KERN_EMERG "[LGSI] Inside %s\n",__func__);
	for (i = 0; i < IMGSENSOR_I2C_DEV_MAX_NUM; i++)
	{
		ret = i2c_add_driver(&gi2c_driver[i]);
		printk(KERN_EMERG "[LGSI] i2c_add_driver returned %d \n",ret);
	}
