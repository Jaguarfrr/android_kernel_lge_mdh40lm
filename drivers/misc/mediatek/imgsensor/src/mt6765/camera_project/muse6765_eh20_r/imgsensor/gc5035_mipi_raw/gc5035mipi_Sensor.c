...
extern char wide_sensor_name[20];/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/
...
#if GC5035_EEPROM_ENABLE
static kal_uint16 read_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC5035_EEPROM_I2C_ADDR);
...
static int read_eeprom_barcode(void)
{
    ...
    if(barcodeFlag != 0x01)
    {
       printk("gc5035 barcodeFlag invalid barcodeFlag =0x%x!\n",barcodeFlag);
        return -1;
    }
    ...
    for (i = 0; i < QRCODE_ADDR_LENS; i++)
    {
        ...
        pr_err("gc5035 sn_code[%d]:%02x,%c",i,sn_code[i],sn_code[i]);
        Sum += sn_code[i];
    }
    pr_err("gc5035 sn_code:%s",sn_code);

    checkSum=Sum%255+1;
    if((checkSum&0xff)==read_eeprom_module(QRCODE_CHECKSUM))
    {
        pr_err("gc5035 barcode checksum sucess\n");
        get_hardware_info_data(HWID_MAIN_CAM_2_SN, sn_code);
        return 0;
    }
    pr_err("gc5035 barcode checksum error\n");
    return -1;
}
...
static int get_eeprom_data(EEPROM_DATA *data)
{
    ...
    otp_data = (u8 *)data;
    for (i=0;i<=0x0E;i++)
        pr_err(" gc5035 [simon]otpdata[0x%x]=0x%x \n", i, *(otp_data + i));
#endif
    read_eeprom_barcode();
    return 0;
}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	pr_info("Exit! cal_shutter = %d, ", cal_shutter);
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(wide_sensor_name, "gc5035_kingcome");/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/
				gc5035_otp_identify();
...
UINT32 GC5035_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("GC5035_MIPI_RAW_SensorInit.\n");
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
