...
static kal_uint16 read_eeprom_module(kal_uint32 addr)
{
	...
	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC02M1_EEPROM_I2C_ADDR);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(macro_sensor_name, "gc02m1macro_kingcome");/*LGE_CHANGE, 2020-11-06, add the camera identifying logic , bk.bae@lge.com*/
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				#if GC02M1_EEPROM_ENABLE
                if (0x6e == imgsensor.i2c_write_id) {
                ...
                } else if (0x20 == imgsensor.i2c_write_id) {
                    hw_info_sub2_otp.sensor_name = "gc02m1b_camera_sensor";
                }
                #endif	
				return ERROR_NONE;
...
UINT32 GC02M1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	printk("GC02M1_MIPI_RAW_SensorInit.\n");
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}
